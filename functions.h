	...
	uint32_t LB(struct InstrFields *Fields,map<uint32_t, uint32_t>& RegisterFile,map <uint32_t, uint32_t>& Memory){
		...
		uint32_t signedbit;
		if (Fields->imm_I11_0 >> 11) signedbit = (Fields->imm_I11_0 | 0xFFFF0000);
		else uint32_t signedbit = (Fields->imm_I11_0 | 0x0000FFFF);
		...
		printf("RegisterFile[Fields->rd]=0x%08x,RegisterFile[Fields->rs1]=0x%08x,signedbit=0x%08x,resultW=0x%08x,resultB=0x%02x,result32=0x%08x\n", RegisterFile[Fields->rd], RegisterFile[Fields->rs1], signedbit, resultW, resultB, result32);
		print_regs();
		return 1;
	};
	...
	int32_t signextendimm;
		...
		print_regs();
                ...
                printf("RegisterFile[Fields->rs1]=0x%08x,RegisterFile[Fields->rd]=0x%08x,effaddress=0x%08x,Memory[effaddress]=0x%08x\n",RegisterFile[Fields->rs1], RegisterFile[Fields->rd],effaddress,Memory[effaddress]);
                print_regs();
		return 1; 
	};
